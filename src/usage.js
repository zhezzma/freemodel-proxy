...
import { applyAccountIp } from './forwarder.js';

const USAGE_URL = 'https://freemodel.dev/api/usage';
const ME_URL = 'https://freemodel.dev/api/auth/me';
...
async function fetchIsAbuser(session, ip, fetchImpl) {
  try {
    const res = await fetchImpl(ME_URL, {
      ...
      headers: applyAccountIp({
        ...
        pragma: 'no-cache',
        cookie: `bm_session=${session}; `,
        referer: 'https://freemodel.dev/dashboard',
      }, ip),
    });
    if (!res.ok) return undefined;
    const data = JSON.parse(await res.text());
    // is_abuser 为 0/1，转成布尔；缺字段时保持 undefined（无法判定）
    if (data?.user && 'is_abuser' in data.user) return Boolean(data.user.is_abuser);
    return undefined;
  } catch {
    return undefined;
  }
...
async function fetchUsage(account, index, fetchImpl) {
  ...
  try {
    ...
    const isAbuser = await fetchIsAbuser(account.session, account.ip, fetchImpl);
    return { ...base, ok: true, usage: JSON.parse(text), isAbuser, fetchedAt: new Date().toISOString() };
  } catch (err) {
