...
.badge.banned { color:#fff; background:var(--red); border-color:var(--red); font-weight:800; }
.card.banned::before { background:var(--red); }
...
function statusFor(account) { if (account.isAbuser === true) return ['已封禁','banned']; const u = account.usage; if (!account.ok) return ['等待/错误','hot']; if (u.window5h?.usedCents >= u.window5h?.limitCents) return ['5h 超额','hot']; if (u.windowWeek?.usedCents >= u.windowWeek?.limitCents) return ['7d 超额','hot']; return ['正常','ok']; }
...
function card(account) { const [label, badge] = statusFor(account); const cardClass = account.isAbuser === true ? 'ok banned' : 'ok'; const meta = (account.tokenTail || '') + (account.sessionUpdatedAt ? ' · session ' + new Date(account.sessionUpdatedAt).toLocaleString('zh-CN') : '') + (account.cachedAt ? ' · 缓存 ' + new Date(account.cachedAt).toLocaleTimeString('zh-CN') : ''); if (!account.ok) return '<article class="card err" id="acc-' + account.id + '"><div class="head"><div><div class="email">' + esc(account.email) + '</div><div class="meta">' + esc(meta) + '</div></div><span class="badge ' + badge + '">' + label + '</span></div><div class="error">' + esc(account.error || (account.hasSession ? '请求中...' : 'missing session')) + '</div></article>'; const u = account.usage; return '<article class="card ' + cardClass + '" id="acc-' + account.id + '"><div class="head"><div><div class="email" title="' + esc(account.email) + '">' + esc(account.email) + '</div><div class="meta">' + esc(meta) + '</div></div><span class="badge ' + badge + '">' + label + '</span></div>' + windowBlock('5小时窗口', u.window5h) + windowBlock('7天窗口', u.windowWeek) + '<div class="metrics"><div class="metric">请求数<b>' + num(u.totalRequests) + '</b></div><div class="metric">Tokens<b>' + num(u.totalTokens) + '</b></div><div class="metric">平均延迟<b>' + num(u.avgLatency) + ' ms</b></div><div class="metric">Cache R/W<b>' + num(u.todayCacheReadTokens) + ' / ' + num(u.todayCacheWriteTokens) + '</b></div></div></article>'; }
...
async function fetchOne(account, accounts) { const res = await fetch(\`/api/accounts/\${encodeURIComponent(account.id)}/usage\`, { cache:'no-store' }); const usage = await res.json(); const merged = { ...account, ...usage, cachedAt: new Date().toISOString() }; if (merged.ok) writeCache(merged); const idx = accounts.findIndex(a => a.id === account.id); if (idx >= 0) { accounts[idx] = merged; render(accounts); observeAccountCards(accounts); } }
function observeAccountCards(accounts) { const pending = new Map(accounts.filter(account => !readCache(account.id)).map(account => [account.id, account])); if (!pending.size) return; const observer = new IntersectionObserver((entries) => { for (const entry of entries) { if (!entry.isIntersecting) continue; const id = entry.target.id.replace(/^acc-/, ''); const account = pending.get(id); if (!account) continue; pending.delete(id); observer.unobserve(entry.target); fetchOne(account, accounts); } }, { rootMargin: '600px 0px' }); for (const id of pending.keys()) { const el = document.getElementById('acc-' + id); if (el) observer.observe(el); } }
async function loadAccounts() { const res = await fetch('/api/accounts', { cache:'no-store' }); const data = await res.json(); const accounts = data.accounts.map(account => { const cached = readCache(account.id); return cached || { ...account, ok:false, error:'滚动到卡片附近时请求...' }; }); render(accounts); observeAccountCards(accounts); }
loadAccounts();
</script>
</body>
</html>`;
